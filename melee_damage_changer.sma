...
SaveAdminFlagCfg()
{
    ...
    if (!fp)
    {
        log_amx("[DMG Changer Melee] Can't write admin flag cfg: %s", g_szAdminFlagCfg);
        return;
    }

    new line[256];

    formatex(line, charsmax(line), "; MDCH admin flag config%c", 10);
    fputs(fp, line);

    formatex(line, charsmax(line), "; Selectable flags via /changedmgsf%c", 10);
    fputs(fp, line);
...
LoadAdminFlagCfg()
{
    ...
    while (!feof(fp))
    {
        ...
        {
            log_amx("[DMG Changer Melee] Unknown admin flag in cfg: %s", line);
        }
        break;
    }
...
public plugin_init()
{
    ...
    register_clcmd("say /changedmgsf",      "Cmd_OpenAdminFlagMenu");
    register_clcmd("say_team /changedmgsf", "Cmd_OpenAdminFlagMenu");

    new datadir[128];
    get_datadir(datadir, charsmax(datadir));
    formatex(g_szCfgPath,       charsmax(g_szCfgPath),       "%s/melee_dmg_changer.cfg", datadir);
    formatex(g_szAdminFlagCfg,  charsmax(g_szAdminFlagCfg),  "%s/mdch_adminflag.cfg", datadir);
...
public Cmd_OpenAdminFlagMenu(id)
{
    if (!is_user_connected(id))
        return PLUGIN_HANDLED;
    ...
    if ( !(get_user_flags(id) & g_RequiredFlag) )
    {
        client_print(id, print_chat, "[DMG Changer Melee] You don't have access to change admin flag!");
        return PLUGIN_HANDLED;
    }

    ShowAdminFlagMenu(id);
    return PLUGIN_HANDLED;
}

ShowAdminFlagMenu(id)
{
    new menu = menu_create("Select Admin Flag (Melee)", "AdminFlagMenuHandler");
    menu_setprop(menu, MPROP_PERPAGE, 7);
    menu_setprop(menu, MPROP_EXITNAME, "Close");
    menu_setprop(menu, MPROP_BACKNAME, "Prev");
    menu_setprop(menu, MPROP_NEXTNAME, "Next");
...
public AdminFlagMenuHandler(id, menu, item)
{
    if (item == MENU_EXIT)
    {
        menu_destroy(menu);
    ...
    client_print(id, print_chat, "[DMG Changer Melee] Admin flag set to: %s.", g_AdminFlagNames[idx]);
